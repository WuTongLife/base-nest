...
// 转换为标准的数据结构
export function transformDataToPaginate<T>(data: { rows: T[]; count: number }): IHttpResultPaginate<T[]> {
  return {
    list: data.rows,
    total: data.count,
  };
...
export class TransformInterceptor<T> implements NestInterceptor<T, THttpSuccessResponse<T>> {
  ...
  intercept(context: ExecutionContext, next: CallHandler<T>): Observable<THttpSuccessResponse<T>> {
    ...
    const code = this.reflector.get<string>(META.HTTP_SUCCESS_RESPONSE_CODE, target) || HttpStatus.OK;
    const msg =
      this.reflector.get<string>(META.HTTP_SUCCESS_RESPONSE_MESSAGE, target) || TEXT.HTTP_DEFAULT_SUCCESS_TEXT;
