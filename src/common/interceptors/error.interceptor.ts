...
export class ErrorInterceptor implements NestInterceptor {
  ...
  intercept(context: ExecutionContext, next: CallHandler<any>): Observable<any> {
    ...
    const code = this.reflector.get<string>(META.HTTP_SUCCESS_RESPONSE_CODE, target) || DEFAULT_ERROR.code;
    const status =
      this.reflector.get<HttpStatus>(META.HTTP_ERROR_RESPONSE_STATUS, target) || HttpStatus.INTERNAL_SERVER_ERROR;
    const msg = this.reflector.get<string>(META.HTTP_ERROR_RESPONSE_MESSAGE, target) || TEXT.HTTP_DEFAULT_ERROR_TEXT;
    return call$.pipe(
      catchError((error) => {
